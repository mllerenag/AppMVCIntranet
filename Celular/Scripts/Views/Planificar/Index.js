...
function buscarProyectos(){
    var retorno = '<table id="tbl_Proyectos" style="width:100%" class="table table-bordered table-hover">';
    retorno += '<thead>';
    retorno += '<th>Cod Proyecto</th>';
    retorno += '<th>Proyecto</th>';
    retorno += '<th>Portafolio</th>';
    retorno += '<th>Fecha Creación</th>';
    retorno += '<th>% Programado</th>';
    retorno += '<th>% Real</th>';
    ...
    var proyecto = $("#vl_txtProyecto").val();
    var categoria = $("#vl_Categoria").val();

    var url_ = $("#UrlServicio").val() + '/api/Planificar/BuscarMonitoreos';
    var entidad_ = { nid_responsable: responsable, co_portafolio : portafolio, co_proyecto: proyecto,  categoria : categoria}
    $.ajax({
        url: url_,
        data: entidad_,
        dataType: 'json',
...
function retornarRegistro(resultado){
    ...
    //if(parametro != 0){
        retorno += '<tr>';
        retorno += '<td>' + resultado.co_proyecto + '</td>';
        retorno += '<td>' + resultado.no_proyecto + '</td>';
        retorno += '<td>' + resultado.no_portafolio + '</td>';
        retorno += '<td>' + resultado.fecha_crea +'</td>';
        retorno += '<td>' + resultado.po_programado +'</td>';
        retorno += '<td>' + resultado.po_real +'</td>';
        retorno += '<td>' + resultado.no_estado +'</td>';
        retorno += '<td>'; 
        retorno +=     '<button type="button" onclick="PopUpDetalle(\'' +  resultado.co_proyecto + '\',\'' + resultado.no_portafolio +  '\',\'' + resultado.po_programado +  '\',\'' + resultado.po_real +  '\',\'' + resultado.co_estado +  '\');"  style="margin-left:10px;margin-right:10px;"class="btn btn-warning">Editar</button>'; 
        retorno +=     '<button type="button" onclick="PopUpConfirmarSolicitud(' + resultado.nid_monitoreo + ',\'' + resultado.co_proyecto +  '\''+',\'' + resultado.co_estado +  '\');" class="btn btn-danger">Ver Gantt</button>'; 
        retorno += '</td>';
        retorno += '</tr>';
    //}
    return retorno;
...
function PopUpPlanificar(parametro, no_estado){
    ...
    $.ajax({
        ...
        type: "post",
        success: function (resultado) {
            PopUpPlanificar_Data(resultado);
        },
        error: function (xhr, status, error) {
            fc_MsjError('001 - Ocurrio un error');
        }
...
function PopUpPlanificar_Data(data){
    $("#myModal1").modal('show');

    var url_ = $("#UrlServicio").val() + '/api/Planificar/CargarFiltroGestion';
    var entidad_ = null;
    $.ajax({
        url: url_,
        data: entidad_,
...
function DatosPopUp(objeto){
    ...
    if(nid_solicitud == null || nid_solicitud == 0){
        ...
        $("#cbo_CategoriaPopUp").val(objeto.nid_prioridad);
...
function Combo_Nivel_PopUp(objeto){
    $("#cbo_NivelPopUp").find('option').remove().end().append('<option value="">--Seleccione--</option>').val('');

    $.each(objeto, function (i, element) {
        $("#cbo_NivelPopUp").append("<option value='" + element.codigo + "'>" + element.nombre + "</option>");
    });
}
...
function ReloadPortafolios(){
    var url_ = $("#UrlServicio").val() + '/api/Planificar/BuscarProyectosPortafolio';
    var responsable = 3;
    var valor = $("#txtNombrePortafolio_Buscar").val();
    var entidad_ = { nid_responsable: responsable, nombre : valor}
    $.ajax({
        url: url_,
        data: entidad_,
        dataType: 'json',
        type: "post",
        success: function (resultado) {
            RetornarTablaProyectosPortafolio(resultado);
        },
        error: function (xhr, status, error) {
            fc_MsjError('001 - Ocurrio un error');
        }
...
function RetornarTablaProyectosPortafolio(lista){
    ...
    if(lista != null){
        for(var x= 0; x < lista.length; x++){
            retorno += '<tr>';
            retorno += '<td>' + lista[x].co_proyecto  + '</td>';
            retorno += '<td>' + lista[x].no_proyecto  + '</td>';
            retorno += '<td>' + lista[x].no_portafolio  + '</td>';
            retorno += '<td>' + '<button type="button" onclick="javascript:AsociarPortafolio(\'' + lista[x].no_proyecto +  '\',\'' + lista[x].co_proyecto+ '\',\'' + lista[x].no_portafolio+ '\',\'' + lista[x].co_portafolio+ '\',\'' + lista[x].nid_proyecto+ '\',\'' + lista[x].nid_portafolio+ '\')" class="btn btn-success">Asignar</button></td>';
            retorno += '</tr>';
        }
    }
    retorno += '</tbody>';
...
function AsociarPortafolio(no_proyecto,co_proyecto,no_portafolio,co_portafolio,idproyecto,idportafolio){

    $("#lblIdProyecto").html(co_proyecto);
    $("#lblNoProyecto").html(no_proyecto);

    $("#lblIdPortafolio").html(co_portafolio);
    $("#lblNoPortafolio").html(no_portafolio);

    $("#hfProyecto").val(idproyecto);
...
function AgregarSolicitudTmp(){
    ...
    var recurso = $("#cbo_CategoriaPopUp").val();
    var recurso_string = $("#cbo_CategoriaPopUp option:selected").text();
...
function Reload_Tabla_Tmp_Solicitudes(){
    var retorno = '<table id="tbl_Tareas_Tmp" style="width:100%" class="table table-bordered table-hover">';
    retorno += '<thead>';
    retorno += '<th>Portafolio</th>';
