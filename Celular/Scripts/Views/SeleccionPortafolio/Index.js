...
function retornarRegistro(resultado){
    ...
    retorno += '<td>' + '<button type="button" onclick="redirectToBalanceo(' +  parametro + ');" class="btn btn-primary">Balanceo</button></td>';
    retorno += '</tr>';
    return retorno;
}

function redirectToBalanceo(parametro){
    if(parametro == 0){
        fc_MsjError('003 - El Portafolio no se encuentra activo');
    }else{
        var entidad_ = { prm : parametro}
        var link = $("#UrlServicioSeleccion").val();
        window.location.href = link + '?prm=' + parametro;
    }
}